...
def search_value_idx(hist, bias = 0):
    for i in range(hist.shape[0]):
        idx = np.abs(bias - i)      #검색위치
        if hist[idx] > 0: return idx       #위치 반환
    return -1                   #대상 없으면 반환

image = cv2.imread("images/dst.jpg",cv2.IMREAD_GRAYSCALE)   #영상읽기
...
idx = np.arange(0,256)
idx = (idx - low) * 255/(high-low)      #수식 적용하여 인덱싱 생성
idx[0:int(low)] = 0
idx[int(high+1):] = 255
