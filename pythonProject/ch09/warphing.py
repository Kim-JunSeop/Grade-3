...
def doWarphing(img1, img2, nFrame, outputs):
        ...
        for h_index in range(0, height):
            for w_index in range(0, width):
                ...
                for i in range(int(np.array(srcP).size / 2)):
                    ...
                    XPrime1 = P1[i] + U * (Q1[i] - P1[i]) + (H * perpendicular(Q1[i] - P1[i])) / np.linalg.norm(
                        Q1[i] - P1[i])
                    XPrime2 = P2[i] + U * (Q2[i] - P2[i]) + (H * perpendicular(Q2[i] - P2[i])) / np.linalg.norm(
                        Q2[i] - P2[i])
                ...
                if (srcX in range(0, width) and srcY in range(0, height)):
                    morphedIm[h_index][w_index] = img1[srcY][srcX]
                else:
                    morphedIm[h_index][w_index] = img1[h_index][w_index]
...
original2 = cv2.rectangle(np.zeros((480, 340, 3), np.uint8), (0, 0), (480, 340), (0, 0, 0), 3, cv2.LINE_4)
nFrame = 1
...
doWarphing(img1, img2, nFrame, outputs)
MsgBox_Success()

plt.subplot(1, 1, 1), plt.imshow(np.uint8(outputs[0]))
plt.title('Step : ' + str(1)), plt.xticks([]), plt.yticks([])
